...
public class ItemController {
    ...
    /**
     * Complete an {@link Item} from a specific bucket and user
     *
     * If the owner id does not match the id of the principal user, item can not be completed.
     *
     * @param ownerId Id of the user that owns the buckets.
     * @param bucketId Id of the bucket.
     * @param itemId Id of the item.
    ...
    public ResponseEntity<?> completeItem(@PathVariable(name = "ownerId") final Long ownerId,
                                                            ...
                                                            @AuthenticationPrincipal final Authentication auth) {
        UserPrincipal currentUser = (UserPrincipal) auth.getPrincipal();
        ItemSummaryResponse response = itemService.completeItem(bucketId, itemId, !Objects.equals(currentUser.getId(), ownerId));
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    /**
     * Uncomplete an {@link Item} from a specific bucket and user
     *
     * If the owner id does not match the id of the principal user, item can not be uncompleted.
     *
     * @param ownerId Id of the user that owns the buckets.
     * @param bucketId Id of the bucket.
     * @param itemId Id of the item.
    ...
    public ResponseEntity<?> uncompleteItem(@PathVariable(name = "ownerId") final Long ownerId,
                                          ...
                                          @AuthenticationPrincipal final Authentication auth) {
        UserPrincipal currentUser = (UserPrincipal) auth.getPrincipal();
        ItemSummaryResponse response = itemService.uncompleteItem(bucketId, itemId, !Objects.equals(currentUser.getId(), ownerId));
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
}
